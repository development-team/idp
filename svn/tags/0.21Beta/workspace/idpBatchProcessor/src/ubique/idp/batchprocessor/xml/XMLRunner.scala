...
*/
object XMLRunner {
  // (directoryName: String, namespace: String, predictedTagPrefix: String)
  ...
  * @param dir - String labels directory name
  * @param namespace - String namespace of tags to collect
  * @param predictedTagPrefix - String prefix to add to predicted tag name
  * @return Tuple2 of HashSet that contains Tuple4 of: prefix, label, qname and predictedTagPrefix_prefix_label
  */ 
  def collectTagNames(directoryName: String, namespace: String, predictedTagPrefix: String): 
  Tuple3[NamesList, NamespaceBinding, Node] = {
    
    val dirFile: File = new File(directoryName)
    var p: ConstructingParser = null
    var rootElem: Elem = null
    var qname: Tuple4[String, String, String, String] = null
    var nB: NamespaceBinding = null
    var completeTree: Node = null
      ...
      aFiles.map[Unit]((aFile) => {
        ...
        if (aFile.isFile()){
          ...
          if (completeTree == null) {
            completeTree = currentTree
            println("Formed complete tree \n" + completeTree )
          } else {
            completeTree = recursiveMerge(currentTree, completeTree)
          }
        }
      })

      completeTree = filterNamespaceTree(completeTree, namespace)
      //if (Constants.DEBUG) println (this.completeTree)
      currentTree = recursiveReverse(completeTree)
  ...
  private def createElementTuple(aElement: Node): Tuple4[String, String, String, String] = {
    createElementTuple(aElement, "")
  }
  
  private def createElementTuple(aElement: Node, predictedTagPrefix: String): Tuple4[String, String, String, String] = {
    new Tuple4(aElement.prefix , aElement.label, aElement.prefix + ":" + aElement.label, predictedTagPrefix+"_"+aElement.prefix+"_"+aElement.label)
  }

...
class Trainer (labelsDir: String, namespace:String, destination:String, 
  ...
  */ 
  private def collectTagNames () = {
    val xmlRRes = XMLRunner.collectTagNames(labelsDir, namespace, predictedTagPrefix)
    this.elementNames = xmlRRes._1
    this.nB = xmlRRes._2
    this.completeTree = xmlRRes._3
    if (Constants.DEBUG) {log.info("Complete tree " + this.completeTree) }
    ...
    if (Constants.DEBUG && this.rootPredicate.isDefined) {
      ...
      log.info("Rules predicate " + this.rootPredicate)
    }
  }
  ...
  * @pram annTuple - Tuple2 of String annotation tag name to create annotator file, Annotator object to save.
  * @return String path where file was saved. 
  */
  private def saveOne(annTuple: Tuple2[String, Annotator]): String = {
  ...
  /**
  *   Saves complete XML tree mined from train set.
  */
  private def saveCompleteTree {
    log.info(" Started to tree file in " + this.destination)
    val dDFile = new File(this.destination)
    var fileName: String = ""
    var aFile: File = null
    try {
  ...
  */
  ...
  def run(): Unit = {
    log.info(" Started collecting tag names for namespace " + this.namespace)

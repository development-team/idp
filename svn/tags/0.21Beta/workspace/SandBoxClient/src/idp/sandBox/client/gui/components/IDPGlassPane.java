...
public class IDPGlassPane extends JComponent implements MouseListener,
		MouseMotionListener {
	...
	private static Logger log = Logger.getLogger(IDPGlassPane.class);
	...
	public IDPGlassPane(double alpha) {
		...
		messageDialog = new IDPMessageDlg("Warning",
	...
	public void setDlgBounds(Rectangle r) {
		...
		if (currentDialog == IDPDialogs.MessageDialog) {
			log.info("messageDialog.setBounds(r.x, r.y, r.width, r.height);");
			messageDialog.setBounds(r.x, r.y, r.width, r.height);
			log.info("messageDialog.updatePreferredSize(r.width, r.height);");
	...
	public void fadeIn(IDPDialogs dialog) {
		...
		if (dialog == IDPDialogs.MessageDialog) {
            ...
            int defaultTestWidth = 190;
            int textWidth = messageDialog.getTextWidth();

            if (textWidth > defaultTestWidth) {
                w += Math.abs(defaultTestWidth - textWidth);
            }
	...
	public void fadeIn(IDPDialogs dialog, String title, String message,
			IDPMessageDlg.messageType messageType) {
		if (currentDialog != IDPDialogs.none)
			return;
	...
	public void fadeOut() {
		...
		if (currentDialog == IDPDialogs.ColorChooser) {
			...
			Rectangle collapsed = new Rectangle(0,0,0,0);
			settingsDialog.setBounds(collapsed);
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1) {
            ...
            if (j != null && j instanceof IDPMessageDlg) {
                rec = j.getBounds();
                bDragged = true;
                mx = e.getX();
                my = e.getY();
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (rec.x > screen_width - 300) {
            rec.x = screen_width - 300;
            if (j != null)
                j.setBounds(rec);
        }
        if (rec.y < 0) {
        ...
        if (rec.y > screen_height - 200) {
            rec.y = screen_height - 200;
            if (j != null)
                j.setBounds(rec);
        }
        j = null;

...
/**
 * Enhanced tabbed pane UI. Have lots of restrictions on account of having been
 * developed for IDP purposes only.
 * 
...
public class IDPTabbedPaneUI extends BasicTabbedPaneUI {
	...
	enum eCloseBtnStatus {
		outside, over, pressed
	}

	/**
	 * Close button status Is changed according to mouse cursor position.
	 */
	...
	protected void installDefaults() {
		...
		try {
			...
			if (imageURL != null) {
				image = new ImageIcon(Toolkit.getDefaultToolkit().getImage(
						imageURL));
			}
			// ////////////////////////////////////////////////////////////
			...
			ImageTail = Transparency.makeColorTransparent(image.getImage(),
					Color.yellow);
			...
			ImageTailW = Transparency.makeColorTransparent(image.getImage(),
					Color.yellow);
			...
			ImageTailRed = Transparency.makeColorTransparent(image.getImage(),
					Color.yellow);
			...
			ImageBG = Transparency.makeColorTransparent(image.getImage(),
					Color.yellow);
			...
			ImageHead = Transparency.makeColorTransparent(image.getImage(),
					Color.yellow);
			...
			ImageHeadU = Transparency.makeColorTransparent(image.getImage(),
					Color.yellow);
			...
			ImageTailU = Transparency.makeColorTransparent(image.getImage(),
					Color.yellow);
	...
	/**
	 * Overridden BasicTabbedPaneUI's method. As against parent method, it
	 * doesn't call paintTabBorder and paintFocusIndicator.
	 * 
	 * @param g
	 * @param tabPlacement
	 * @param rects
	...
	protected void paintTab(Graphics g, int tabPlacement, Rectangle[] rects,
		...
		// ////////////////////////////////////////////////////////////////
		// modify rectangles that are painted after selected tab cause it has
		// tail
	...
	private void paintAdditionalButton(Graphics g) {
		...
		/*
		 * int count = ((IDPTabbedPane)tabPane).getHiddenTabsCount(); if (count
		 * > 0) { g.setColor(new Color(255,255,255,100)); Rectangle r =
		 * tabPane.getBounds(); rect = new Rectangle(r.width - 35, 2, 30, 18);
		 * g.fillRect(rect.x, rect.y, rect.width, rect.height);
		 * g.setColor(Color.black); Font f = new Font("Verdana", Font.BOLD, 8);
		 * g.setFont(f); g.drawString(">>"+Integer.toString(count), rect.x, 10);
		 * } else rect = null;
		 */
	}
	...
	protected void paintTabBackground(Graphics g, int tabPlacement,
		...
		if (tabIndex == tabPane.getSelectedIndex()) {
			...
			Image ImageResized = ImageBG.getScaledInstance(rectBG.width,
					rectBG.height, Image.SCALE_REPLICATE);
			try {
				MediaTracker media = new MediaTracker(tabPane);
			...
			// ////////////////////////////////////////////////////////////
			g.drawImage(ImageHead, rect.x, rect.y, imageHeadWidth,
					imageHeadHeight, null);
			// ////////////////////////////////////////////////////////////
			...
			if (bConsoleWindow == true) {
				...
				// ////////////////////////////////////////////////////////
				// chose what tail to draw according to mouse position status
				// (close button status)
			...
			// g.setColor(Color.red);
			// g.drawRect(curCloseBtnRect.x, curCloseBtnRect.y,
			// curCloseBtnRect.width, curCloseBtnRect.height);
			...
			g2D.drawLine(r.x, r.y + TAB_HEIGHT + 1, rect.x, r.y + TAB_HEIGHT
					+ 1);
			g2D.drawLine(rect.x + rect.width + 24, r.y + TAB_HEIGHT,
					r.width - 3, r.y + TAB_HEIGHT);
			// ////////////////////////////////////////////////////////////
			...
			// ////////////////////////////////////////////////////////////
			if (tabIndex < tabPane.getSelectedIndex()) {
				g.drawImage(ImageHeadU, rect.x, rect.y - 1, imageHeadWidthU,
						imageHeadHeightU, null);

				Rectangle rectNext = rects[tabIndex + 1];
				g.drawLine(rect.x + imageHeadWidth, rect.y - 2,
						rectNext.x + 10, rect.y - 2);
			} else {
				g.drawLine(rect.x - 5, rect.y - 2, rect.x + rect.width,
						rect.y - 2);
	...
	/**
	 * Return tab width. Overridden to make some changes.
	 * 
	 * @param tabPlacement
	 * @param tabIndex
	 * @param metrics
	...
	/**
	 * Return label X shift. Overridden to make some changes.
	 * 
	 * @param tabPlacement
	 * @param tabIndex
	 * @param isSelected
	...
	/**
	 * Mouse motion listener. Updates close button picture (to white or red).
	 * 
	...
	/**
	 * Mouse listener. Changes the close button status and fire close event when
	...
	class IDPMouseListener implements MouseListener {
		...
		public void mouseReleased(MouseEvent arg0) {
			...
			if (closeBtnStatus == eCloseBtnStatus.pressed
				...
				if (tabPane instanceof IDPTabbedPane) {
					((IDPTabbedPane) tabPane).fireCloseTabEvent(arg0,
							selectedTabIndex);
	 ...
	 * 
	 * @param x
	 *            - the <b>x</b> position of mouse cursor
	 * @param y
	 *            - the <b>y</b> position of mouse cursor
	 * @return index of tab
	 */
	...
	/**
	 * Check if cursor is above close button rectangle. Update close button
	 * status if needed.
	 * 
	 * @param x
	 *            - the <b>x</b> position of mouse cursor to check
	 * @param y
	 *            - the <b>y</b> position of mouse cursor to check
	 * 
	 * @see #getCloseRect
	 */
	...
	/**
	 * Get close button location as rectangle for specific tab that presented as
	 * input parameter <b>rect</b>
	 * 
	 * @param rect
	 *            - tab's rectangle for which the method should return location

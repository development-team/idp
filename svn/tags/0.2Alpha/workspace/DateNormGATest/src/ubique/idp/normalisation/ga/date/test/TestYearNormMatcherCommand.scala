...
object TestYearNormMatcherCommand {
  ...
  val propYear2 = Prop.forAll(year2)( y => {
    ...
    val dm = new DateModelMatcher("20 jun " + strY);
    dm.setCursor(2)
    prog.setApplicationData(dm)
    yearCommand.execute_void(aProgram, 0, null)
    val res = aProgram.getIndividual().getApplicationData().asInstanceOf[DateModelMatcher].getYear() 
    if (y > currentYear) {
      // print("\n 1900 " + y + " " + res)
      (1900 + y) == res
    } else {
  ...
  val propYear4 = Prop.forAll(year4)( y => {
    val dm = new DateModelMatcher("20 jun " + y);
    dm.setCursor(2)
    prog.setApplicationData(dm)
    yearCommand.execute_void(aProgram, 0, null)
    val res = aProgram.getIndividual().getApplicationData().asInstanceOf[DateModelMatcher].getYear()
